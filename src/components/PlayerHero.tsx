...
const scrollToBottom = () => {
    // Scrolls the viewport down by exactly one screen height (100vh), 
    // which should land the next content section at the top.
  ...
  return (
    <div className="relative min-h-0 h-screen overflow-hidden"> 
      ...
      {/* Scroll Down Arrows - Positioned on right side with vertical spacing */}
      <div className="absolute top-1/2 right-8 transform -translate-y-1/2 flex flex-col items-center gap-3 z-10 cursor-pointer" onClick={scrollToBottom}>
      ...
      {/* Container: TIGHTEST vertical padding: py-1 */}
      <div className="relative container mx-auto px-6 py-1">
        ...
        <div className="flex justify-between items-center mb-3 animate-fade-in"> 
          ...
          <div className="flex gap-3">
            ...
            <Button 
              variant="ghost" 
              size="icon" 
              className={`text-foreground/80 hover:text-success hover:bg-success/10 transition-all duration-300 hover:scale-110 ${!isMuted ? 'text-success animate-pulse' : ''}`}
              onClick={toggleMute}
            >
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
          ...
          {/* Left: Player Visual - LARGE SIZE RETAINED */}
          <div className="relative flex justify-center animate-fade-in" style={{ animationDelay: '0.2s' }}>
            <div className="relative">
              <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 w-[400px] h-[400px] bg-[radial-gradient(circle,hsl(var(--primary)/0.4),hsl(var(--primary)/0.1)_40%,transparent_70%)] animate-glow-pulse blur-xl" />
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-96 h-2 rounded-full bg-gradient-to-r from-transparent via-primary to-transparent animate-pulse blur-sm" />
              
              {/* Player card container: LARGE SIZE RETAINED */}
              <div className="relative w-[380px] h-[520px] flex justify-center group perspective-1000 mt-auto"> 
                <div className="relative w-full h-full rounded-2xl overflow-hidden border-4 border-primary/50 transition-all duration-700 group-hover:border-primary group-hover:scale-105 group-hover:shadow-[0_0_80px_hsl(var(--primary)/0.6)] group-hover:rotate-y-5">
                  ...
                  {/* Overlays, Scan line, Corner accents */}
                ...
                <div className="absolute -right-8 top-[10%] bg-primary/20 backdrop-blur-md border-2 border-primary/50 rounded-lg p-3 animate-float shadow-lg shadow-primary/20">
                  <Zap className="w-6 h-6 text-primary" />
                  <div className="text-xs font-display text-primary mt-1">PWR</div>
                </div>
                <div className="absolute -left-8 top-[20%] bg-accent/20 backdrop-blur-md border-2 border-accent/50 rounded-lg p-3 animate-float shadow-lg shadow-accent/20" style={{ animationDelay: '1s' }}>
          ...
          {/* Right: Player Info (SLIGHTLY ENLARGED, TIGHTLY COMPRESSED) */}
          <div className="space-y-4 animate-fade-in" style={{ animationDelay: '0.4s' }}>
            
            {/* Name section: Reverting to a more attractive font size */}
            <div className="space-y-2">
              <h1 className="text-4xl md:text-5xl font-display font-black text-transparent bg-clip-text bg-gradient-to-r from-primary via-primary-glow to-accent glow-text uppercase tracking-tight leading-tight transition-all duration-500 hover:scale-105">
              ...
              <p className="text-lg text-muted-foreground font-body tracking-wide">{fullName}</p> 
              <div className="flex items-center gap-3 flex-wrap">
                <Badge className="bg-success/30 text-success border-2 border-success/60 px-4 py-1.5 font-display font-black text-base uppercase tracking-wider transition-all duration-300 hover:scale-110 hover:bg-success/40 hover:shadow-lg hover:shadow-success/30">
                  {role}
                </Badge>
                <span className="text-xl text-primary/50">|</span>
                <span className="text-foreground/90 font-body font-semibold text-lg tracking-wide">{country}</span>
              </div>
            </div>

            {/* Stats Display - INCREASED SIZE */}
            <div className="grid grid-cols-2 gap-3">
              <div className="gradient-card rounded-xl p-4 border-2 border-border/50 hover-lift transition-all duration-500 hover:border-primary hover:shadow-[0_0_30px_hsl(var(--primary)/0.4)] group relative overflow-hidden">
                <div className="relative">
                  <div className="text-sm text-muted-foreground font-body mb-2 uppercase tracking-widest flex items-center gap-1">
                    <div className="w-1.5 h-1.5 bg-primary rounded-full animate-pulse" />
                    Overall
                  </div>
                  <div className="text-4xl md:text-5xl font-display font-black text-primary glow-text transition-all duration-300 group-hover:scale-110">{overall}</div> 
                  <div className="text-xs text-primary/60 font-display mt-1 uppercase tracking-wider">Rating</div>
                </div>
              </div>
              
              <div className="gradient-card rounded-xl p-4 border-2 border-border/50 hover-lift transition-all duration-500 hover:border-accent hover:shadow-[0_0_30px_hsl(var(--accent)/0.4)] group relative overflow-hidden">
                <div className="relative">
                  <div className="text-sm text-muted-foreground font-body mb-2 uppercase tracking-widest flex items-center gap-1">
                    <div className="w-1.5 h-1.5 bg-accent rounded-full animate-pulse" style={{ animationDelay: '0.5s' }} />
                    Potential
                  </div>
                  <div className="text-4xl md:text-5xl font-display font-black text-accent glow-text-accent transition-all duration-300 group-hover:scale-110">{potential}</div>
                  <div className="text-xs text-accent/60 font-display mt-1 uppercase tracking-wider">Max</div>
                </div>
              </div>
            </div>

            {/* Detailed Info Grid - INCREASED SIZE */}
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1 p-3 bg-muted/30 backdrop-blur-sm rounded-lg border border-border/40 transition-all duration-300 hover:bg-muted/40 hover:border-primary/50 hover:scale-105 group">
                <div className="text-xs text-muted-foreground font-body uppercase tracking-widest">Born</div>
                <div className="text-sm font-display font-bold text-foreground group-hover:text-primary transition-colors">{birthDate}</div>
              ...
              <div className="space-y-1 p-3 bg-muted/30 backdrop-blur-sm rounded-lg border border-border/40 transition-all duration-300 hover:bg-muted/40 hover:border-primary/50 hover:scale-105 group">
                ...
                <div className="text-2xl font-display font-black text-foreground group-hover:text-primary transition-colors">{age}</div> 
              </div>
              <div className="space-y-1 p-3 bg-muted/30 backdrop-blur-sm rounded-lg border border-border/40 transition-all duration-300 hover:bg-muted/40 hover:border-accent/50 hover:scale-105 group">
                <div className="text-xs text-muted-foreground font-body uppercase tracking-widest">Batting</div>
                <div className="text-sm font-display font-bold text-foreground group-hover:text-accent transition-colors">{battingStyle}</div>
              </div>
              <div className="space-y-1 p-3 bg-muted/30 backdrop-blur-sm rounded-lg border border-border/40 transition-all duration-300 hover:bg-muted/40 hover:border-accent/50 hover:scale-105 group">
                <div className="text-xs text-muted-foreground font-body uppercase tracking-widest">Bowling</div>
                <div className="text-sm font-display font-bold text-foreground group-hover:text-accent transition-colors">{bowlingStyle}</div>
              </div>
            </div>

            {/* Teams - INCREASED SIZE */}
            <div className="bg-muted/40 backdrop-blur-sm rounded-xl p-4 border-2 border-border/40 transition-all duration-300 hover:bg-muted/50 hover:border-primary/50">
              <div className="text-xs text-muted-foreground font-body mb-3 uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {teams.map((team, idx) => (
                  <Badge 
                    key={idx}
                    className="bg-background/60 backdrop-blur-sm text-foreground border-2 border-border/50 px-3 py-1 font-body text-sm transition-all duration-300 hover:bg-primary/20 hover:border-primary/60 hover:scale-105 hover:shadow-lg hover:shadow-primary/20"
                  >
                    {team}
                  </Badge>
                ))}
